...
impl Resolver {
    ...
    pub fn resolve(&mut self, stmt: &Stmt) -> Result<(), String> {
        match stmt {
            ...
            Stmt::Print { expression } => self.resolve_expr(expression, expression.get_id())?,
            Stmt::ReturnStmt {
                keyword: _,
                value: None,
            } => (),
    ...
    fn resolve_expr(&mut self, expr: &Expr, resolve_id: usize) -> Result<(), String> {
        match expr {
            ...
            Expr::Call {
                ...
                //self.resolve_expr(callee.as_ref(), resolve_id)?;
    ...
    fn resolve_expr_var(&mut self, expr: &Expr, resolve_id: usize) -> Result<(), String> {
        match expr {
            ...
            Expr::Call {
                ...
                Expr::Variable { id: _, name } => {                println!("Resolving {name:?} with id {resolve_id}");self.resolve_local(&name, resolve_id)},
    ...
    fn resolve_local(
        ...
        for i in (0..=(size - 1)).rev() {
            ...
            println!("Checking {}", size-i-1);
            if scope.contains_key(&name.lexeme) {
                // println!("Name: {}, ID: {}, Dist: {}", name.lexeme, id_to_use, size-1-i);
                println!("Resolving id {resolve_id} to have distane {}", size-1-i);
                self.interpreter
                    .borrow_mut()
